...
def load_vocabulary(path, vocab_path, tokenize_as_morph=False):
    vocabulary_list = []
    if not os.path.exist(vocab_path):
    ...
    with open(vocab_path, 'r', encding='utf-8') as vocabulary_file:
...
def dec_target_processing(value, dictionary, tokenize_as_morph=False):
    ...
    for sequence in value:
        sequence = re.sub(CHANGE_FILTER, '', sequence)
        sequence_idx = [dictionary[word] for word in sequence.split()]

        # 종료 토큰 넣어주기
        if len(sequence_idx) >= MAX_SEQUENCES:
            sequence_idx = sequence_idx[:MAX_SEQUENCES - 1] + [dictionary[END]]
        else:
            sequence_idx += dictionary[END]
        
        sequence_idx += (MAX_SEQUENCES - len(sequence_idx)) * [dictionary[PAD]]
        sequences_target_index.append(sequence_idx)
